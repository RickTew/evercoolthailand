...
**DRESS REHEARSAL — PASSED WITH REAL EMAIL (2026-07-12 night).** `test.evercoolthailand.com` fully verified in Resend (subdomain MX/DKIM/SPF at A2 Zone Editor; harmless scaffolding, dies with A2). Proven on EC-10020: real iPad mail + 1.1MB photo → ticket with attachment; app reply sent from `hi@test...` and DELIVERED receipt stamped by events webhook; customer reply threaded back onto the same ticket (quoted text collapsed). Fixes shipped during the test: Vercel functions pinned to `sin1` (vercel.json — was iad1 vs Singapore DB = the slowness), `.email-light` palette pin (dark mode made inbox pale-on-white), "Reply sent." banner (no provider name). KNOWN polish items (post go-live): inline `cid:` images show broken icon in rich body (file itself arrives as attachment chip); inbox perf beyond region fix; **EM-DASH AUDIT: Rick's golden rule = never use em dashes anywhere; sweep all user-visible strings in the ported email UI and replace them.** **Remaining for go-live: (1) root MX flip in A2 Zone Editor — EDIT existing root MX `0 mail.evercoolthailand.com` → `10 inbound-smtp.ap-northeast-1.amazonaws.com`, then Verify in Resend; (2) staff logins/profiles for the 6 staff; (3) exports of A2 mailboxes (can happen after flip — old mail stays readable at A2 until cancellation).**
