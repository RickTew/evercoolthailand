...
import { DialButton } from "@/components/public/RevealPhone";
  ...
  useEffect(() => {
    ...
    function frame() {
      ...
      for (const p of parts) {
        ...
        grad.addColorStop(0, "rgba(0,160,190,0)");
        grad.addColorStop(1, "rgba(0,160,190,0.25)");
...
export default function HomeEngineered() {
  ...
  return (
    <main className={`${archivo.variable} home-eng bg-[var(--he-bg)] text-[var(--he-ink)]`}>
      ...
      <section className="relative overflow-hidden border-b border-[var(--he-line)]">
        ...
        <div className="relative px-5 md:px-12 pt-20 pb-16 md:pt-28 md:pb-24">
          ...
          >
            {c.headline1}
            <br />
            <span className="text-[var(--he-accent)]">{c.headline2}</span>
          </h1>
          <p className="max-w-xl text-[15px] md:text-base leading-relaxed text-[var(--he-muted)] mb-10">
            {c.sub}
          </p>
          <div className="flex flex-wrap gap-3">
            <Link
              href="/quote"
              className="bg-[var(--he-accent)] hover:bg-[var(--he-accent-hover)] text-[var(--he-accent-contrast)] font-bold text-sm px-6 py-3.5 rounded-none transition-colors"
            >
              {c.ctaQuote} →
            </Link>
            <Link
              href="/products"
              className="border border-[var(--he-line2)] hover:border-[var(--he-accent)] text-[var(--he-ink)] font-semibold text-sm px-6 py-3.5 rounded-none transition-colors"
            >
              {c.ctaProducts}
            </Link>
          </div>
        ...
        {/* Stats rail */}
        <div className="relative border-t border-[var(--he-line)] grid grid-cols-2 md:grid-cols-4">
          {[
            ...
            <div
              key={s.l}
              className={`px-5 md:px-12 py-5 ${i > 0 ? "border-l border-[var(--he-line)]" : ""} ${i > 1 ? "border-t md:border-t-0" : ""}`}
            >
              <p
              ...
              <p className="text-[11px] uppercase tracking-widest text-[var(--he-muted)] mt-1">{s.l}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* ── Spec rows ── */}
      <section className="px-5 md:px-12 py-16 border-b border-[var(--he-line)]">
        <div className="flex items-baseline justify-between mb-8 gap-4">
          ...
          <p className="text-xs text-[var(--he-muted)] hidden md:block">{c.specSub}</p>
        </div>
        <div className="border-t border-[var(--he-line)]">
          {SPEC_ROWS.map((row, i) => (
            <Link
              key={row.href + i}
              href={row.href}
              className="group grid grid-cols-[auto_1fr_auto] md:grid-cols-[80px_1fr_1fr_auto] items-baseline gap-4 border-b border-[var(--he-line)] py-5 hover:bg-[var(--he-panel)] transition-colors px-2 -mx-2"
            >
              <span className="text-xs tabular-nums text-[var(--he-muted)]">{String(i + 1).padStart(2, "0")}</span>
              <span className="text-base md:text-lg font-bold group-hover:text-[var(--he-accent)] transition-colors">
                {lang === "th" ? row.th : row.en}
              </span>
              <span className="hidden md:block text-xs text-[var(--he-muted)]">{row.detail}</span>
              <span className="text-[var(--he-accent)] opacity-0 group-hover:opacity-100 transition-opacity">→</span>
            </Link>
          ))}
        </div>
      </section>
      ...
      {/* ── Factory band ── */}
      <section className="grid md:grid-cols-2 border-b border-[var(--he-line)]">
        ...
        <div className="px-5 md:px-12 py-12 md:py-16 flex flex-col justify-center md:border-l border-[var(--he-line)]">
          <h2
          ...
          <p className="text-sm leading-relaxed text-[var(--he-muted)] max-w-md mb-8">{c.factoryBody}</p>
          <div>
            <p className="text-[11px] uppercase tracking-widest text-[var(--he-muted)] mb-3">{c.certsHeading}</p>
            <div className="flex flex-wrap gap-x-5 gap-y-2">
              {CERTS.map((cert) => (
                <span key={cert} className="text-sm font-semibold text-[var(--he-ink)] tabular-nums">
                  {cert}
                </span>
              ))}
            </div>
      ...
      <section className="px-5 md:px-12 py-16 border-b border-[var(--he-line)]">
        ...
        <div className="grid md:grid-cols-2 gap-px bg-[var(--he-line)] border border-[var(--he-line)]">
          {PROJECTS.map((p) => (
            <div key={p.name} className="bg-[var(--he-bg)] p-6 md:p-8">
              <p className="text-lg font-bold">{p.name}</p>
              <p className="text-[11px] uppercase tracking-widest text-[var(--he-accent)] mb-3">{p.place}</p>
              <p className="text-sm leading-relaxed text-[var(--he-muted)]">{lang === "th" ? p.th : p.en}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="px-5 md:px-12 py-20 text-center">
        ...
        <p className="text-sm text-[var(--he-muted)] mb-8">{c.finalBody}</p>
        <div className="flex flex-wrap justify-center gap-3">
          <Link
            href="/quote"
            className="bg-[var(--he-accent)] hover:bg-[var(--he-accent-hover)] text-[var(--he-accent-contrast)] font-bold text-sm px-8 py-4 transition-colors"
          >
            {c.finalCta} →
          </Link>
          <DialButton
            className="border border-[var(--he-line2)] hover:border-[var(--he-accent)] text-[var(--he-ink)] font-semibold text-sm px-8 py-4 transition-colors"
            ariaLabel={c.call}
          >
            {c.call}
          </DialButton>
