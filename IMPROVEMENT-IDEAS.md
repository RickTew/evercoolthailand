...
- **Advisors run 2026-07-12** — Security: 0 errors, 12 warnings. Performance: 0 errors, 20 warnings (all "Auth RLS Initialization Plan" — wrap `auth.uid()`/`current_setting()` in `(select …)` so they evaluate once per query, not per row; low urgency at current volume) + 16 info suggestions. Security warnings, by priority:
  - **[FIXED 2026-07-12] Direct public INSERT on `bookings` / `contact_messages` / `quotes`** — policies "Anyone can insert booking", "Anyone can insert contact message", "quotes_public_insert" had `WITH CHECK (true)` for role public/anon, letting anyone POST rows straight to PostgREST and bypass the honeypot/length caps. Dropped via SQL Editor (all inserts now go through the service-role API routes). Forms unaffected.
  - **[toggle] Leaked Password Protection disabled** — enable in Auth settings (blocks known-breached passwords).
  - **[review, don't blind-fix] SECURITY DEFINER functions `is_admin()` / `is_eq_user()` callable by public/signed-in** — likely used inside RLS policies, so revoking EXECUTE could break RLS; review before changing.
  - **[hardening] Function search_path mutable** on `is_admin`, `is_eq_user`, `generate_referral_code` — `ALTER FUNCTION … SET search_path = ''` (need exact signatures).
  - **[low] Public bucket `gallery-images` allows listing** — a broad SELECT policy lets clients list all files; restrict if unpublished images shouldn't be enumerable.
