...
- **Advisors run 2026-07-12** — Security: 0 errors, 12 warnings. Performance: 0 errors, 20 warnings (all "Auth RLS Initialization Plan" — wrap `auth.uid()`/`current_setting()` in `(select …)` so they evaluate once per query, not per row; low urgency at current volume) + 16 info suggestions. Security warnings, by priority:
  - **[fix] Direct public INSERT on `bookings` / `contact_messages` / `quotes`** — policies "Anyone can insert booking", "Anyone can insert contact message", "quotes_public_insert" have `WITH CHECK (true)` for role public/anon. The app inserts via service role, so these are unused and let anyone POST rows straight to PostgREST, bypassing the honeypot/length caps. Drop them (SQL provided to Rick).
